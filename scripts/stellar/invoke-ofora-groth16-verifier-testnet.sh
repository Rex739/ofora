...
set -e
printf '%s\n' "$output"
node -e "const fs=require('fs'); fs.writeFileSync('$ARTIFACT_DIR/groth16-direct-transaction.json', JSON.stringify({network:'$NETWORK', verifierContractId:'$contract_id', status:Number(process.argv[1]), output:process.argv[2]}, null, 2)+'\n')" "$status" "$output"
exit "$status"
